...
TEST_CASE("opening shared memory") {
  SECTION("creating a message queue while at it") {
    ...
    auto shared_memory = shm::open(name, read_only | create, fs::perms::all);
    CHECK(shared_memory);
  ...
  SECTION("creating an already existing queue") {
    ...
    auto dup = shm::open(name, read_only | create | exclusive, 0666);
    CHECK_FALSE(dup);
  ...
  SECTION("opening non existing queue without creating it") {
    auto const name    = NAME;
    auto shared_memory = shm::open(name, read_only);
